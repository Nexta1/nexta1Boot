...
import com.nexta1.core.filter.JwtAuthenticationTokenFilter;
 ...
 *
 * @author nexta1
 * @see UserDetailsService#loadUserByUsername  用于登录流程通过用户名加载用户
...
public class SecurityConfig {
    ...
    public AuthenticationEntryPoint unauthorizedHandler() {
        ...
        return (request, response, exception) -> {
            ResponseDTO<Object> responseDTO = ResponseDTO.fail(ErrorCode.Client.COMMON_NO_AUTHORIZATION, request.getRequestURI());
    ...
    public LogoutSuccessHandler logOutSuccessHandler() {
        return (request, response, authentication) -> {
            ...
            if (loginUserDetail != null) {
                ...
                // 删除用户缓存记录
                redisCache.loginUserCache.delete(loginUserDetail.getCachedKey());
                // 记录用户退出日志
